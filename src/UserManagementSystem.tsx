...
const EditUserModal = ({ user, plans, onClose, onSaved }: { user: UserRow; plans: Plan[]; onClose: () => void; onSaved: () => void }) => {
  ...
  const [activeSection, setActiveSection] = useState<'plan' | 'status' | 'role' | 'tokens'>('plan');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        {/* Toast */}
        {toast && (
          <div className={`mx-4 mt-3 px-3 py-2 rounded-xl text-xs font-semibold flex items-center gap-2 ${toast.ok ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-red-50 text-red-700 border border-red-200'}`}>
...
export const UserManagementSystem: React.FC = () => {
  ...
  const saveTokenPolicies = async () => {
    ...
    try { await adminFetch('/api/admin/save-token-policies', { method: 'POST', body: JSON.stringify({ policies: editedPolicies }) }); fetchTokenPolicies(); }
  ...
  const filteredUsers = users.filter(u => {
    ...
    return matchSearch && (filterPlan === 'all' || u.subscription?.plan_id === filterPlan) && (filterStatus === 'all' || u.profile?.account_status === filterStatus);
  });
  ...
  return (
    <div className="w-full">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-0.5">Manage all users — plans, roles, tokens, and passwords.</p>
      </div>
      ...
      {error && (
        <div className="mb-4 bg-red-50 border border-red-200 rounded-xl px-4 py-3 flex items-center gap-3 text-red-700 text-sm">
          ...
          <div className="flex-1">
            <p className="font-bold text-xs">Failed to load</p>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    {['User', 'Plan', 'Status', 'Role', 'Specialty', 'Token Usage', 'Password Reset', 'Joined', 'Edit'].map(h => (
            ...
            {plans.length === 0 ? (
              ...
              return (
                <div key={plan.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                  ...
                  <p className="text-[10px] text-gray-400 mt-1.5">Saved: {policy?.monthly_tokens || policy?.trial_tokens || '—'}</p>
                </div>
              );
            })}
          </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  {['User ID', 'Plan', 'Status', 'Trial', 'Start Date', 'End / Expiry'].map(h => (
              ...
              <tbody>
                ...
                {subscriptions.length === 0 && <tr><td colSpan={6} className="text-center py-16 text-gray-400 text-sm">No subscriptions found.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
      ...
      {/* Edit Modal */}
      {editingUser && <EditUserModal user={editingUser} plans={plans} onClose={() => setEditingUser(null)} onSaved={() => { fetchUsers(); }} />}
    </div>
  );
};
